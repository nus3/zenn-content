...
- ローカル npm パッケージではバージョニング管理が不要
  - 共通モジュールの変更を各チームに即座に反映できる

一方で気になる点としては次の点です。

- npm パッケージの依存管理に使われる lock ファイルは 1 つに集約される
- バージョンニングがないので、共通モジュールに破壊的変更を入れづらい

1 つ目の気になる点としてあげた lock ファイルが 1 つに集約される点については、各チームで頻繁に依存パッケージの追加、更新をする際にコンフリクトがどの程度発生するのか、また、このコンフリクトにより各チームで独立したフロントエンド開発がどの程度、阻害されるのかは考慮が必要です。

この点に関しては、例えば pnpm の場合、lock ファイルのコンフリクトは手動で解消せずに、[`pnpm install`を実行することで自動で解消したり](https://pnpm.io/git#merge-conflicts)、[Git Branch Lockfiles](https://pnpm.io/ja/git_branch_lockfiles)といった機能を使うことでブランチごとに lock ファイルを分けることで、開発時にはコンフリクトを回避し、別のタイミングで分けた lock ファイルをマージすることが可能なようです。こういった仕組みを使うことで、lock ファイルのコンフリクトを気にせず、各チームが独立してフロントエンド開発を進められるかもしれません。

また、もう一つの気になる点としてあげた、バージョンニングがないので共通モジュールに破壊的変更を入れづらい点に関してはトレードオフの観点もあると思っています。バージョニングがないことで共通モジュールの変更が即座に反映できるというメリットを取るか、開発の独立性を高めるために npm パッケージとして公開するかは状況や要件によって変わります。

継続した運用を考えるのであれば、npm パッケージとして公開するのか、ローカル npm パッケージとして扱うのかは、両方のメリット、デメリットも踏まえた意思決定が必要になるでしょう。
