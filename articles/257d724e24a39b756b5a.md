...
仕組みとしては storycap で storybook に登録しているコンポーネントのスクショをとり、前回のスクショと比較して見た目が変わっていないかをチェックしてくれる
...
workflows:
  run_all:
    jobs:
      - visual_regression:
          name: visual regression testing for prod
          context: staging.lycle.gmb
          requires:
            - build
          filters:
            tags:
